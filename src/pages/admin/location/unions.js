    ...
    <AdminLayout>
            <div className='main-wraper flex'>
                <div className='page-wraper'>
                    ...
                    <div className='p-3'>
                        <Breadcum title='Unions' route='admin/users' />
...
export async function getServerSideProps({ query }) {
    ...
    if (page) {
        const res = await Axios.get(`${process.env.ADMIN_URL}/location/unions?page=${page}`)
        const data = res.data
        const links = res.data.links
        // Pass data to the page via props
        return { props: { data, links } }
    } else {
        const res = await Axios.get(`${process.env.ADMIN_URL}/location/unions`)
        const data = res.data
        const links = res.data.links
        // Pass data to the page via props
        return { props: { data, links } }
